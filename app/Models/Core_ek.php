...
class Core_ek extends Model
{
    ...
    public static function serveByRegister($register, $min, $max)
    {
        ...
        $data['core_ek_aggregate']['rit'] = self::whereBetween('created_at', [$min, $max])
                    ->where('register', 'like', $register.'%')
                    ->count('barcode_antrian');
        
        $data['core_ek_aggregate']['brix'] = self::whereBetween('created_at', [$min, $max])
                    ->where('register', 'like', $register.'%')
                    ->avg('brix_nira');
        
        $data['core_ek_aggregate']['pol'] = self::whereBetween('created_at', [$min, $max])
                    ->where('register', 'like', $register.'%')
                    ->avg('pol_nira');
        
        $data['core_ek_aggregate']['rendemen'] = self::whereBetween('created_at', [$min, $max])
                    ->where('register', 'like', $register.'%')
                    ->avg('rendemen');
